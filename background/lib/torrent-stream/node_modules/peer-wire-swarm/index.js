...
var HANDSHAKE_TIMEOUT = 50000;
var CONNECTION_TIMEOUT = 50000;
...
var join = function(port, swarm) {
    ...
    if (!pool) {
        ...
        console.log("CREATING NETCONNECTION");
...
Swarm.prototype._drain = function() {
    ...
    console.log("Getting peer for addr");
    console.log(addr);
    var peer = this._peers[addr];
    if (!peer) return;
    ...
    if(this.utp && !peer.noUtp) {
    	...
    	// XXX
    	// UNCOMMENT THIS LINE TO RUN WITHOUT PROXY
    	// connection = net.connect(parts[1], parts[0]);

    	// XXX UNCOMMENT THIS SECTION TO CONNECT TO A LOCAL SOCKS5
    	connection = proxysocket.create('localhost', 9050);
    ...
    // console.log("CONNECTION ISSSSSSSSSSSSS");
    // console.log(connection);

    // while(connection._connecting){
...
Swarm.prototype._onwire = function(connection, wire) {
    ...
    connection.on('data', function(){
    	console.log("ON DATA ECHO FUNCTION");
    });
    wire.on('end', cleanup);
