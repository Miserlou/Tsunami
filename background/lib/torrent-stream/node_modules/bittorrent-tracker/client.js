...
function Tracker (client, announceUrl, opts) {
  ...
  if (protocol === 'udp:') {
    console.log("Got UDP BOO");
    self._requestImpl = self._requestUdp
  } else if (protocol === 'http:' || protocol === 'https:') {
    console.log("Got http or https");
...
Tracker.prototype._announce = function (opts) {
  ...
  console.log("Announcing to ");
  console.log(self._announceUrl);
  console.log(opts);
...
Tracker.prototype._requestHttp = function (requestUrl, opts) {
  ...
  console.log("Gonn request");
  console.log(requestUrl + '?' + common.querystringStringify(opts));
  console.log(self.client._infoHash);

  get.concat(requestUrl + '?' + common.querystringStringify(opts), function (err, data, res) {
    if (err) { console.log(err); console.log("is the error"); return self.client.emit('warning', err) }
    if (res.statusCode !== 200) { console.log(res.statusCode); console.log(res); return self.client.emit('warning', new Error('Non-200 response code ' + res.statusCode + ' from ' + requestUrl)) }
    if (data && data.length) { console.log("got data"); self._handleResponse(requestUrl, data) }
  })
}
